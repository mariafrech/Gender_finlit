...
function [v_max, idx] = max_future_value_last_work(budget, A_prime, z, r, t, par, V_ret)
    ...
    %for zp = 1:par.Nz
        for rp = 1:par.Nr_risky
            %EV = EV + par.Pi(z,zp) * par.Pi_risk(r,rp) * V_ret{zp,rp,1};
            EV = EV + par.Pi_risk(r,rp) * V_ret{z,rp,1};
        end
   % end
