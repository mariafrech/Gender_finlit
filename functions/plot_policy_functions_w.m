function plot_policy_functions_w(sav_w, inv_w, cons_w, par, t)
    % t is the time index to plot (e.g., par.Twork - 10 for 10 periods before retirement)
    ...
    for fig = 1:2
        ...
        for z = 1:par.Nz
            ...
            for r = 1:par.Nr_risky
                mean_sav = mean_sav + reshape(sav_w{z, r, t}, par.Na, par.Na_risk);
                mean_inv = mean_inv + reshape(inv_w{z, r, t}, par.Na, par.Na_risk);
                mean_cons = mean_cons + reshape(cons_w{z, r, t}, par.Na, par.Na_risk);
            end
        ...
        saveas(gcf, fullfile(output_folder, sprintf('mean_policy_functions_w_%s', strrep(figure_titles{fig}, ' ', '_'))));
        close(gcf);
    end
end